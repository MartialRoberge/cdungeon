  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: "#080810", border: "1px solid #1e1e35" }}>
        ...
        <div className="p-1">
          {payload.lines.map((l) => {
            const lineNum = parseInt(l.id, 10);
            return (
              <motion.div
                key={l.id}
                onMouseEnter={() => { if (!disabled) setHovered(l.id); }}
                onMouseLeave={() => setHovered(null)}
                onClick={() => { if (!disabled) { sounds.click(); onAnswer(lineNum); } }}
                animate={hovered === l.id && !disabled ? { backgroundColor: "rgba(255,51,85,0.07)" } : { backgroundColor: "transparent" }}
                transition={{ duration: 0.15 }}
                className="flex items-center gap-3 px-3 py-1.5 rounded"
                style={{
                  cursor: disabled ? "default" : "pointer",
                  borderLeft: hovered === l.id && !disabled ? "2px solid rgba(255,51,85,0.5)" : "2px solid transparent",
                }}
              >
                <span className="text-xs w-5 text-right shrink-0 select-none font-mono" style={{ color: "#3a3a5c" }}>{l.id}</span>
                <span className="text-sm font-mono flex-1 whitespace-pre" style={{ color: "#a0a0c0" }}>{l.text}</span>
                {hovered === l.id && !disabled && (
                  <motion.span
